...
class SmartScopeTest {
    ...
    fun `v2 suffix in cruds order is accepted`() {
        ...
        assertEquals(
            SmartScope.Fhir(
                ...
                "Patient",
                setOf(Interaction.READ, Interaction.SEARCH),
            ),
            scope,
        )
    }
    ...
    fun `registering known scopes succeeds`() {
        val scopes =
            parseRegisteredScopes(listOf("openid", "fhirUser", "launch", "patient/Patient.rs"))
    ...
    fun `serialize re-emits fhir scopes in canonical cruds order regardless of parse order`() {
        ...
        val fhir =
            SmartScope.Fhir(
                ScopeContext.PATIENT,
                "Patient",
                setOf(Interaction.SEARCH, Interaction.READ),
            )
